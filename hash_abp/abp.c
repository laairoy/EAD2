...
ABP * inserir_chave(int chave, ABP **abp)
{
	...
	if(chave == (*abp)->chave) return NULL;
	else if(chave < (*abp)->chave) return inserir_chave(chave, &((*abp)->esq));
...
ABP * buscar_chave_abp(int chave, ABP *abp)
{
	...
	if(abp->chave == chave) return abp;

	if(chave > abp->chave) return buscar_chave_abp(chave, abp->dir);
	return buscar_chave_abp(chave, abp->esq);
}